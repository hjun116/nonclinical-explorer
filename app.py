...
─────────────────────────
APIs used (all free except Claude):
  • PubMed E-utilities  — https://eutils.ncbi.nlm.nih.gov/
  • ChEMBL REST API     — https://www.ebi.ac.uk/chembl/api/data/
  • Claude API          — classification via anthropic SDK

Deploy: Streamlit Cloud → set ANTHROPIC_API_KEY in App secrets
"""
...
def classify_papers_claude(papers: list[dict], query: str) -> list[dict]:
    """Classify papers using Claude API in batches of 15."""
    import anthropic
    key = get_api_key()
    if not key:
        return keyword_classify_all(papers)
    ...
    for i in range(0, len(papers), CHUNK):
        ...
        prompt = f"""You are a pharmaceutical scientist specializing in nonclinical drug development.

Drug/Query: "{query}"

Classify each paper into exactly one nonclinical category and extract key structured fields.
Categories:
  safety   — toxicology, genotoxicity, carcinogenicity, safety pharmacology
  pk       — pharmacokinetics, ADME, bioavailability, metabolism, drug interaction
  pd       — pharmacodynamics, MOA, target binding, selectivity profiling
  efficacy — in vivo disease model efficacy, therapeutic effect in animals
  other    — reviews, methodology, in vitro only, unrelated

Papers (indexed from {i}):
{abstracts}
...
[
  {{
    "index": <number>,
    "category": "<safety|pk|pd|efficacy|other>",
    "species": "<animal species or empty string>",
    "key_metric": "<most important quantitative finding or empty string>",
    "model": "<disease model or test type or empty string>",
    "confidence": "<high|low>"
  }}
]"""
        try:
            ...
            for r in results:
                ...
                if 0 <= idx < len(chunk):
                    p = chunk[idx]
                    p["category"]   = r.get("category", "other") if r.get("category") in ["safety","pk","pd","efficacy","other"] else "other"
                    p["confidence"] = "high" if r.get("confidence") == "high" else "low"
                    findings = []
                    if r.get("species"):    findings.append({"key": "Species",    "val": r["species"]})
                    if r.get("key_metric"): findings.append({"key": "Key metric", "val": r["key_metric"]})
                    if r.get("model"):      findings.append({"key": "Model/Test", "val": r["model"]})
                    p["key_findings"] = findings
        except Exception:
            keyword_classify_all(chunk)

    return papers


# ══════════════════════════════════════════════════════════════
# KEYWORD CLASSIFIER (fallback when no API key)
# ══════════════════════════════════════════════════════════════
RULES = {
    "safety": {
...
def main():
    ...
    st.caption("Preclinical data explorer powered by PubMed · ChEMBL name normalization · Claude AI classification")
    ...
    with st.sidebar:
        st.header("Settings")
        st.caption("Claude API key enables AI-powered classification. Without it, keyword-based classification is used.")
        ...
        st.divider()
        st.caption("**Classification mode**")
        if get_api_key():
            st.success("AI classification (Claude)")
        else:
            st.warning("Keyword classification (fallback)")
    ...
    if search and query:
        ...
        with st.status("Searching...", expanded=True) as status:
            ...
            # 3. Classify
            if get_api_key():
                papers = classify_papers_claude(papers, query)
            else:
